...
function App() {
  ...
  const cercaPokemon = async () => {
    ...
    // 1. Cerchiamo il match nella lista (anche parziale)
    let nomeReale = listaPokemon.find(p => p.toLowerCase() === query);
    
    // Se scrivi solo "Toxtricity", lui trova la prima variante disponibile
    if (!nomeReale) {
      nomeReale = listaPokemon.find(p => p.toLowerCase().startsWith(query + " ("));
    }

    if (!nomeReale) {
      alert("Pokémon non trovato. Prova a selezionarlo dal suggerimento mentre scrivi!");
      return;
    }
    ...
    if (pkm) {
      ...
      // Trova tutte le forme basandosi sul nome prima della parentesi
  ...
  const scambiaForma = async (idPkm, nuovaForma) => {
    ...
    if (data) {
      setSquadra(squadra.map(p => {
        if (p.id === idPkm) {
          const diffVitality = data.Vitality - p.Vitality;
          return {
            ...p, ...data,
            curHP: Math.max(1, p.curHP + diffVitality),
            currentForm: nuovaForma, availableForms: p.availableForms 
          };
        }
        return p;
      }));
